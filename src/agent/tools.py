...
class BallSettings(BaseModel):
    ...
    ball_number: int = Field(description="Ball number in sequence (1-8)")
    ...
    feed: float = Field(description="Time to next ball in seconds (0.8-5.0)", ge=0.8, le=5.0)
...
class DrillCard(BaseModel):
    ...
    duration: str = Field(description="Duration for this drill (e.g., '5 min', '10 min')")
    machine_position: str = Field(description="Machine placement (e.g., 'Baseline Center', 'Back Glass Center')")
    ball_sequence: list[BallSettings] = Field(description="1-8 ball settings in sequence")
    sequence_repetitions: int = Field(description="Number of times to repeat the ball sequence (1-20)", ge=1, le=20)
...
def generate_training_session(session: TrainingSession) -> str:
    ...
    **What to provide:**
    ...
    2. drills: 4-6 DrillCard objects with ball sequences
       ...
       - machine_position: Where to place machine (e.g., "Baseline Center", "Back Glass Center")
       - ball_sequence: 1-8 BallSettings objects (each ball's parameters)
       - sequence_repetitions: How many times to repeat the sequence (1-20)
       - focus_points: 2-3 technique tips
       - training_plan_id: Leave empty (auto-generated)

    **Ball sequence design:**
    Each ball in ball_sequence should have:
    - ball_number: 1, 2, 3, etc.
    - spin_type: "Topspin", "No Spin", or "Underspin"
    - spin_strength: 0-10 (use formulas: beginner=2, intermediate=4, advanced=6, elite=8)
    - speed: 0-10 (use formulas: beginner=4, intermediate=6, advanced=8, elite=10)
    - drop_point: -10 to +10 (negative=left, positive=right; e.g., -6=backhand corner, +6=forehand corner)
